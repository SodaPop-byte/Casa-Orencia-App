...
router.get('/', async (req, res) => {
  try {
    ...
    // 2. Check for a 'category' filter
    if (req.query.category) {
      filter.category = req.query.category;
    }

    // 3. --- NEW: Check for a 'search' filter ---
    if (req.query.search) {
      // Add a search filter for the 'name' field
      // $regex provides partial matching (like SQL 'LIKE')
      // $options: 'i' makes the search case-insensitive
      filter.name = { $regex: req.query.search, $options: 'i' };
    }
    
    // 4. Use the filter object (it might have category, search, both, or neither)
...
// --- PUT (UPDATE) A PRODUCT (No changes) ---
